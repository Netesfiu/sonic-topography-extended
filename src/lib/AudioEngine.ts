...
const AUDIO_KEYS: Array<keyof AudioData> = [
  'bass','mid','treble','energy','subBass','lowMid','highMid','presence','brilliance','air',
...
export class AudioEngine {
  ...
  private musicState: MusicState = {
    ...
    isDownbeat: false, beatPulse: 0, onset: 0, lowOnset: 0, midOnset: 0,
    highOnset: 0, build: 0, drop: 0, stereoPan: 0, stereoWidth: 0,
    shortEnergy: 0, longEnergy: 0, onsetRate: 0,
  };

  public isWallpaperEngineMode(): boolean { return this.wallpaperAudioReceived; }
  ...
  private processAudioFrame(now: number, dt: number) {
    let sum = 0, centroidNum = 0, volatility = 0, leftEnergy = 0, rightEnergy = 0, stereoDifference = 0;
    for (let i = 0; i < 64; i++) {
    ...
    if (this.isPlaying) { this.lastActiveTime = now; this.idleStartTime = 0; }
    else this.lastIdleTime = now;
    ...
    if (totalOnset.strength > 0.12 && energy > this.idleEnergyThreshold) {
      if (this.onsetTimes.length === 0 || now - this.onsetTimes[this.onsetTimes.length - 1] > 110) {
    ...
    if (this.pulseTrigger.enabled && this.pulseTrigger.mode === 'Advanced') {
    ...
    } else if (this.pulseTrigger.enabled && (!this.rhythmSyncEnabled || this.tempoConfidence < this.tempoConfidenceThreshold) && lowOnset.strength > 0.08) {
      ...
      if (now - this.lastRawPulseTime >= cooldownMs) {
        ...
        this.onFreqTrigger?.(Math.max(0.08, lowOnset.strength) * this.pulseTrigger.pulseStrength * 2.5 * this.beatTriggerStrength, 'Kick', 'Pulse');
      }
    }

    if (this.pulseTrigger.enabled && this.pulseTrigger.mode === 'Auto Beat' && midOnset.strength > 0.22 && lowOnset.strength < 0.15 && now - this.lastSnareTime > 220) {
      this.lastSnareTime = now;
      this.onFreqTrigger?.(midOnset.strength * 0.16 * this.beatTriggerStrength, 'Snare', 'Pulse');
    }

    if (this.meteorTrigger.enabled) {
      if (this.meteorTrigger.mode === 'Advanced') this.evaluateAdvancedTrigger(this.meteorTrigger, now);
    ...
    const sharpness = Math.max(0, brightness - this.prevBrightness) * 8 + highOnset.strength * 0.8;
    ...
    const density = bands.reduce((count, value) => count + (value > activeThreshold ? 1 : 0), 0) / bands.length;
    const spectralCentroid = sum > 1e-6 ? centroidNum / sum : 0;

    this.targetData = { bass: legacyBass, mid: legacyMid, treble: legacyTreble, energy, subBass, lowMid, highMid, presence, brilliance, air, warmth, brightness, sharpness, smoothness, density, spectralCentroid };
  ...
  private evaluateAdvancedTrigger(config: TriggerConfig, now: number) {
    ...
    this.onFreqTrigger?.(value * Math.max(0.1, config.pulseStrength) * this.beatTriggerStrength, 'Advanced', config.action);
  }
  ...
  private estimateTempo(now: number) {
    ...
    for (const candidate of intervals) {
      const index = Math.max(0, Math.min(scores.length - 1, Math.round((candidate.bpm - 70) * 2)));
      for (let delta = -2; delta <= 2; delta++) {
        const bin = index + delta;
        if (bin >= 0 && bin < scores.length) scores[bin] += candidate.weight * Math.exp(-0.7 * delta * delta);
      }
      totalWeight += candidate.weight;
    }
    let bestIndex = 0, bestScore = 0;
    for (let i = 0; i < scores.length; i++) if (scores[i] > bestScore) { bestScore = scores[i]; bestIndex = i; }
  ...
  private correctBeatPhaseFromOnset(now: number, strength: number) {
    ...
    if (this.beatAnchorMs <= 0) { this.beatAnchorMs = now; return; }
  ...
  private updateBeatClock(now: number) {
    if (!this.rhythmSyncEnabled || this.beatPeriodMs <= 0 || this.tempoConfidence < 0.18 || this.beatAnchorMs <= 0 || !this.isPlaying) {
    ...
    if (beatNumber > this.lastEmittedBeatNumber) {
      if (this.lastEmittedBeatNumber >= 0 && beatNumber - this.lastEmittedBeatNumber > 2) this.lastEmittedBeatNumber = beatNumber - 1;
      ...
      if (this.pulseTrigger.enabled && this.pulseTrigger.mode === 'Auto Beat') {
        ...
        const strength = (0.08 + this.renderedData.bass * 0.8 + this.latestLowOnset * 0.5) * this.pulseTrigger.pulseStrength * downbeatBoost * this.beatTriggerStrength;
  ...
  public getAudioData(deltaTime: number = 0.016): AudioData {
    ...
    this.musicState = { ...this.musicState, bpm: this.bpm, tempoConfidence: this.tempoConfidence, onset: this.latestOnset, lowOnset: this.latestLowOnset, midOnset: this.latestMidOnset, highOnset: this.latestHighOnset, build: this.build, drop: this.drop, stereoPan: this.stereoPan, stereoWidth: this.stereoWidth, shortEnergy: this.shortEnergy, longEnergy: this.longEnergy, onsetRate };
    return output;
  }
  ...
  public getStereoSpectrum() {
    return { left: new Float32Array(this.leftSpectrum), right: new Float32Array(this.rightSpectrum), mono: new Float32Array(this.monoSpectrum) };
  }
  ...
  public getIdleWaveIntensity(deltaTime: number = 0.016): number {
    ...
    if (this.wallpaperAudioReceived) {
      if (this.isPlaying) { targetIntensity = 0; this.idleStartTime = 0; }
      else {
        if (this.idleStartTime === 0) this.idleStartTime = now;
        targetIntensity = (now - this.idleStartTime) / 1000 >= this.debounceDuration ? 1 : 0;
      }
    } else targetIntensity = this.isPlaying ? 0 : 1;
    const fadeSpeed = 1 / Math.max(0.05, this.idleFadeOutDuration);
    const delta = fadeSpeed * Math.max(0, deltaTime);
    if (targetIntensity > this.currentIdleIntensity) this.currentIdleIntensity = Math.min(targetIntensity, this.currentIdleIntensity + delta);
    else if (targetIntensity < this.currentIdleIntensity) this.currentIdleIntensity = Math.max(targetIntensity, this.currentIdleIntensity - delta);
    return this.currentIdleIntensity;
  }

  public setIdleWaveDebounce(seconds: number) { this.debounceDuration = Math.max(0, seconds); }
  public setIdleFadeOutDuration(seconds: number) { this.idleFadeOutDuration = Math.max(0.05, seconds); }
}
